...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('cb-authorize')
        .setDescription('(RESTRICTED) Authorize a user to use the /cb-set command.')
        .addUserOption(option =>
            option.setName('user')
                .setDescription('Select the user to authorize.')
                .setRequired(true)),
    async execute(interaction) {
        ...
        if (authorizedUsers.includes(user.id)) {
            await interaction.reply({ content: `User <@${user.id}> is already authorized.`, ephemeral: true });
        } else {
            authorizedUsers.push(user.id);

            try {
                fs.writeFileSync(authorizedUsersFilePath, JSON.stringify(authorizedUsers, null, 2));
                await interaction.reply({ content: `User <@${user.id}> has been authorized to use the /cb-set command.`, ephemeral: true });
            } catch (error) {
