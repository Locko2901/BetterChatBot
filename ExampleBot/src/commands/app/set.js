...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('cb-set')
        .setDescription('(RESTRICTED) Add a channel to the list of allowed channels.')
        .addChannelOption(option =>
            option.setName('channel')
                .setDescription('Select the channel to add.')
                .setRequired(true)),
    async execute(interaction) {
        ...
        if (channels.allowedChannels[serverId] && channels.allowedChannels[serverId].includes(channelId)) {
            await interaction.reply(`Channel <#${channelId}> is already in the list of allowed channels.`);
            return;
        }

        if (!channels.allowedChannels[serverId]) {
            channels.allowedChannels[serverId] = [];
        }
        channels.allowedChannels[serverId].push(channelId);

        try {
            fs.writeFileSync(channelsFilePath, JSON.stringify(channels, null, 2));
            await interaction.reply(`Channel <#${channelId}> added to the list of allowed channels.`);
        } catch (error) {
